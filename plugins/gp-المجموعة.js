...
`;
  ...
  await conn.groupSettingUpdate(m.chat, isClose).then(async (_)=> {
	  m.reply(`⚠️ *_تم ${isClose == 'announcement' ? 'إغلاق' : 'فتح'} المجموعة ${args[1] ? `لمدة *${clockString(timeoutset)}_*` : ''}`);
  });
  if (args[1]) {
	 setTimeout(async () => {
...
handler.help = ['grouptime *<فتح/إغلاق>* *<المدة>*'];
handler.tags = ['مجموعة'];
handler.command = /^(المجموعة|المجموعه)$/i;
