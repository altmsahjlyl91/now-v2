...
const handler = async (m, { command, usedPrefix, conn, text }) => {
  ...
  if (command === 'المشغل' && (device == 'desktop' || device == 'web')) throw `*[❗] Los mensajes de botones aun no estan disponibles en WhatsApp web, acceda a su celular para poder ver y usar los mensajes con botones.*`;
  if (enviando) return;
  enviando = true;
  ...
  try {
    ...
    if (!text.includes('SN@') && command !== 'المشغل') await conn.sendMessage(m.chat, { text: dataMessage }, { quoted: m });      
...
handler.command = /^(play.1|play.2|المشغل$/i;
